"""Qualities and Ratings: Jev judging an Article on a developer-declared spectrum.
...
model and key once, producing a ``BoundQuality``, the one place that turns an Article
into a ``Rating``. Several bound Qualities are rated against one subject in a single
Jev request by :func:`rate`; one Score question per Quality, keyed by the Quality key.
...
class Quality:
    ...
    def bind(self, model, key: str) -> "BoundQuality":
        ...
        return BoundQuality(model=model, key=key, instructions=self.instructions, levels=tuple(self.levels))
...
class BoundQuality:
    ...
    def rate(self, subject: Article, *, client: TypeSafeClient | None = None) -> Rating | None:
        """Rate ``subject`` on this Quality alone. ``None`` when there is nothing to rate."""
...
def rate(
    ...
    *,
    ...
    """Rate ``subject`` on every Quality in one Jev request; Ratings come back in the same order.
    ...
    for quality in qualities:
        ...
        ratings.append(Rating(key=quality.key, levels=quality.levels, probabilities=probabilities))
    return ratings


def _is_blank(subject: Article) -> bool:
    return not subject.title.strip() and not subject.body.strip()


def _state(subject: Article) -> dict[str, dict[str, str]]:
    """What Jev reads for a Rating: title and body only, so unrelated material stays out."""
    body = subject.body[: get_setting("WAGTAIL_JEV_MAX_CHARS")]
    return {"article": {"title": subject.title, "body": body}}
